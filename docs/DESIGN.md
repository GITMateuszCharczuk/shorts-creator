...
- **Purpose:** **Format-aware compositor** (ADR 0007 / **0007a**) — a `pure(render_manifest)`
  resolve step binds the format's `layout` template (hybrid region data + a closed primitive/
  animation library on the `LayoutEngine` adapter) and the engine paints frames at **30 fps**, then
  mix VO & music + **finishing polish** → final 9:16 MP4. Engine shared with data-viz; **CPU raster**
  (deterministic) + **NVENC** encode.
