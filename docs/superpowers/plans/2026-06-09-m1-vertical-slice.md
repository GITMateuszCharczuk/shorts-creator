...
**Compliance boundary notes (cross-milestone):** (a) **`primary_keyword`** is a 00b *deliverable* per spec Ch.4 — M1's 00b emits it into the script and 02 checks it; it stays optional in `script.schema` for M0's fixtures but is expected on real 00b output. (b) **Caption design** (spec lists it under M3) is *pulled forward* here as Stage 03's baseline `.ass` styler (Task 8/9 — font/stroke/shadow/emphasis-color/safe-zone); M3 refines it with per-niche brand-kit tokens (`emphasis_hex`). (c) **Per-format target length** (ADR 0006 D1) is a 00b field owned by the **M3** format library — M1's single-render slice doesn't size to a lane; deferred, not dropped.
