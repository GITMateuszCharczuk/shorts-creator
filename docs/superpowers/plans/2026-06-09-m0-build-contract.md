...
- **M0's contracts are LIVING, not frozen (the cross-plan rule).** The 11 schemas, the **stage-manifest edge table** (Task 12/13 — each stage's `inputs[]`/`outputs[]`), the **adapter Protocols** (`ModelBackend`/`DistributionAdapter`, Task 9), and the **golden fixtures** are the *interface* every later milestone builds against — and later milestones legitimately evolve them. The binding rule: **any milestone that changes a stage's inputs/outputs, a schema's shape, or a Protocol's signature/methods MUST, in the same task, (1) update the M0 declaration (schema file / stage manifest / Protocol), (2) bump that schema's `schema_version`, (3) update the affected golden fixture(s), and (4) re-run the drift-catcher + the golden-fixture sweep so CI stays green.** The drift-catcher (Task 12) and the fixture sweep (Task 5) are the enforcement — they will fail loudly if a later milestone forgets. Known in-arc evolutions tracked this way: `qc` shape (M5), `posts` fields/states (M5), `feature_record` `ramp_label`/`creative_qc_overall`/`niche` (M5/M6), `05b` inputs + `Stage 05` `music` re-add (M3/M5), `03` `word_timings` (M1), `tts_segments`/`vlm_judge` on `ModelBackend` (M3), the `DistributionAdapter` exactly-once redesign (M5), `script.platform_meta.ai_disclosure` (M5), `render_manifest` as a declared `05`→`05x`/`05b` edge (M2/M3/M5), the `quality.floor` config block (M6). Each is the owning milestone's responsibility to land *with* its M0-sync.
